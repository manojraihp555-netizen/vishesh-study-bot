...
def search_notes(query):
    ...
    try:
        cursor.execute("""
            SELECT id, student_class, subject, topic, file_id, file_type, file_name
            FROM notes
            WHERE LOWER(topic) LIKE LOWER(?) OR LOWER(subject) LIKE LOWER(?) OR LOWER(student_class) LIKE LOWER(?)
...
def delete_note(topic):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("DELETE FROM notes WHERE LOWER(topic) = LOWER(?)", (topic,))
        conn.commit()
...
def get_all_notes():
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("SELECT id, student_class, subject, topic, file_id, file_type, file_name FROM notes ORDER BY student_class ASC, subject ASC, topic ASC")
        return cursor.fetchall()
    except sqlite3.Error as e:
